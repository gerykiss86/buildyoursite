...
class ClaudeExecutor:
    ...
    def execute_command(self, prompt, job_id=None, stream=False):
        """Execute claude command with the given prompt"""
        if not job_id:
            job_id = str(uuid.uuid4())

        # Build the command
        # Using su - to get full environment for clauderunner
        command = [
            'su', '-', 'clauderunner', '-c',
...
def execute():
    ...
    try:
        ...
        if stream:
            ...
            def generate():
                for output in executor.execute_command(prompt, job_id, stream=True):
                    yield output
            ...
            result = executor.execute_command(prompt, job_id, stream=False)
...
def test_endpoint():
    ...
    test_prompt = "Output only this text without using any tools: Hello from Claude Execution Server"
    result = executor.execute_command(test_prompt, stream=False)
    return jsonify({
        'test': True,
        'prompt': test_prompt,
        'result': result
